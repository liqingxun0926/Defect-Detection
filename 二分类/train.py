# -*- coding: utf-8 -*-
"""
Created on Fri Feb 28 22:01:53 2020
...
batch_size = 32#每次输入的图片数目

#Prepare input data
classes = os.listdir('training_data')#获取分类的总类别
num_classes = len(classes)#本文为2

# 20% of the data will automatically be used for validation
validation_size = 0.2#在training_data总数据集中选取20%作为验证集
img_size = 128#图片的大小，相当于caffe中的crop_size
num_channels = 3
...
def create_weights(shape):
    return tf.Variable(tf.truncated_normal(shape, stddev=0.05)) #权重大小,初始值
...
#这样设置pool层特征图大小是上一层conv层的一半w2= (上一层大小-核的大小)/步长 +1
...
def train(num_iteration):#总迭代数相当于caffe中max_iter
...
train(num_iteration=3000)#总迭代数相当于caffe中max_iter